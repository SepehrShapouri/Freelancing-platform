...
function Proposal() {
    ...
    console.log(project)
    if(!project?.proposals?.length) return(
    <div className='w-full h-full bg-sky-50 dark:bg-slate-700 p-4'>
        <h2 className='text-2xl w-full flex items-center justify-between font-bold text-cyan-800 dark:text-indigo-400'>پیشنهادی روی این پروژه ثبت نشده <ArrowLeft className='text-cyan-600 dark:text-indigo-400' onClick={()=>moveBack()}/></h2>
    </div>
    )
  return (
...
<div className="relative overflow-x-auto shadow-md sm:rounded-lg ">
    <table className="w-full text-sm text-left rtl:text-right">
     ...
     نام فریلنسر
        ...
        <tbody>
          {project?.proposals.map((proposal)=><FreelancerProposalRows key={proposal._id} proposal={proposal}/>)}
        </tbody>
    </table>
</div>
    </div>
